...
class BCMAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = self.gcn_q(get_graph_feature(q, k=self.k_num))  # 16×72×1000×1
...
class BEBlock(nn.Module):
    def __init__(self, channels, r=4, k_num=8):
        ...
        self.global_att0 = nn.AdaptiveAvgPool2d(1)

        self.local_att = DGCNN_Layer(self.k_num, channels)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        random_tensor = torch.randn(1, 128, 1000, 1).cuda()
        x1 = self.project_be(random_tensor)

        x_local = self.project_knn(x1)
        x_local = self.local_att(get_graph_feature(x_local, k=self.k_num))

        # a=self.global_att0(x1)# b*128*1*1
...
class DS_Block(nn.Module):
    def __init__(self, initial=False, predict=False, out_channel=128, k_num=8, sampling_rate=0.5, clusters=200):
        ...
        self.in_channel = 6 if self.initial is True else 6
        ...
        if predict == False:
            with torch.no_grad():
                x_out = torch.gather(x[:, :, :, :4], dim=2,
                                     index=indices.repeat(1, 1, 1, 4))  # x_out剪枝后保留的x[32,1,1000,4]
    ...
    def forward(self, x):
        ...
        #
        # out = self.embed_00(out)  # out[32,128,2000,1] [32,128,1000,1] # resnet模块

        out = self.resfomer_1(x)  # BCMA模块

        # x_down = self.down_1(out)
        # x2 = self.l1(x_down)
        # x_up = self.up_1(out, x2)  # OA输出模块
        #
        # out = torch.cat([x1, x_up], dim=1)
        ...
        #     return x_ds, y_ds, w1, [w0_ds, w1_ds]
        # else:
        #     w1_ds, indices = torch.sort(w1, dim=-1, descending=True)
        #     w1_ds = w1_ds[:, :int(N * self.sr)]
...
class BCLNet(nn.Module):
    def __init__(self):
        ...
        self.ds_0 = DS_Block(initial=True, predict=False, out_channel=72, k_num=9,
                             sampling_rate=0.5, clusters=150)  # sampling_rate=0.5
        # self.ds_1 = DS_Block(initial=False, predict=True, out_channel=128, k_num=6, sampling_rate=0.5, clusters=150)

    def forward(self, x):
        B, _, N, _ = x.shape

        out = self.ds_0(x)  # 返回的是x_ds, y_ds, [w0, w1], [w0_ds, w1_ds]
...
#
# bcl_layer = BCLNet()
# feature = torch.Tensor(np.random.rand(16, 1, 1000, 6))
# y = torch.Tensor(np.random.randint(2, size=(16, 1000)))
# output = layer(feature, y)
