...
from bclnet import BCLNet
from common import knn, rigid_transform_3d
from mscb import MSCB
from pos import PosE_Initial
from tmc import TMC
...
class Matcher():
    def __init__(self,
        ...
        self.pose = PosE_Initial(6, 72, 100, 1000).to(self.device)

        self.classification = nn.Sequential(
            nn.Conv1d(72, 32, kernel_size=1, bias=True),
        ...
        self.layer = MSCB(72, 72).to(self.device)

        self.cab = nn.Sequential(
            nn.Linear(72, int(72 / 4)),
            nn.ReLU(inplace=True),
            nn.Linear(int(72 / 4), 72),
            nn.Sigmoid()  # 添加 Sigmoid 激活函数用于二分类任务
        ).to(self.device)

        self.sab = nn.Sequential(
            nn.Conv2d(72, int(72 / 4), kernel_size=7, padding=3),
            nn.BatchNorm2d(int(72 / 4)),
            nn.ReLU(inplace=True),
            nn.Conv2d(int(72 / 4), 72, kernel_size=7, padding=3),
            nn.BatchNorm2d(72)
        ).to(self.device)

        dims = [[240], [76], [216], [47], [64], [6]]
        views = len(dims)
        self.tmc = TMC(1, views, dims, 50).to(self.device)

        self.self_attention = SelfAttention(72, num_heads=8).to(self.device)

        self.bcl_layer = BCLNet().to(self.device)

        self.weight = nn.Parameter(torch.Tensor(72, 72)).to(self.device)
    ...
    def SC2_PCR(self, corr_pos, src_keypts, tgt_keypts, testing, gt_labels, epoch):
        """
        Input:
            - src_keypts: [bs, num_corr, 3]
            - tgt_keypts: [bs, num_corr, 3]
        ...
        out = self.pose(corr_pos.permute(0, 2, 1))
        #
        # out = self.self_attention(out.permute(0, 2, 1)).permute(0, 2, 1)
        #
        # out = out.permute(0, 2, 1)
        #
        # out = self.cab(out) * out
        #
        # out = out.unsqueeze(0).permute(0, 3, 1, 2)
        #
        # out = (self.sab(out) * out).squeeze(0).permute(1, 0, 2)

        # out = torch.matmul(out.permute(0, 2, 1), self.weight).permute(0, 2, 1)
        out = torch.bmm(out, SC_measure)

        out = self.layer(out.permute(1, 0, 2).unsqueeze(0)).squeeze(0).permute(1, 0, 2)

        out = self.bcl_layer(out.unsqueeze(-1)).squeeze(-1)

        confidence = self.classification(out).permute(0, 2, 1).squeeze(-1)

        # evidences, evidence_a, losses = self.tmc(confidence, gt_labels.to(torch.int64), epoch)
        #
        # confidence = (evidences.squeeze(-1) + evidence_a.squeeze(-1)) / 2
        loss = 0
