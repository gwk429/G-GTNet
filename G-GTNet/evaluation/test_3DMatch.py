    ...
    with torch.no_grad():
        for i in tqdm(range(num_pair)):
            ...
            corr, src_keypts, tgt_keypts, gt_trans, gt_labels = \
                corr.to(model.device), src_keypts.to(model.device), tgt_keypts.to(model.device), gt_trans.to(model.device), gt_labels.to(model.device)
            data = {
                'corr_pos': corr,
...
def eval_3DMatch(model, config, use_icp):
    ...
    for scene_ind, scene in enumerate(scene_list):
        dset = ThreeDMatchTest(root='F:/gwk/PointDSC/data/3DMatch',
                               descriptor='fcgf',
        ...
        scene_vals[0] = scene_stats.mean(0)
    ...
    for scene, stats in all_stats.items():
        ...
        #              )
        print(scene_ind, float(scene_vals[scene_ind, 0] * 100), float(scene_vals[scene_ind, 1]),
              float(scene_vals[scene_ind, 2]),
              float(scene_vals[scene_ind, 6]), float(scene_vals[scene_ind, 7]), float(scene_vals[scene_ind, 8]),
              float(scene_vals[scene_ind, 9]), float(scene_vals[scene_ind, 10]))
        data_frame = pd.DataFrame(
            data={'scene': scene_ind,
                  'recall': float(scene_vals[scene_ind, 0] * 100),
                  're': float(scene_vals[scene_ind, 1]),
                  'te': float(scene_vals[scene_ind, 2]),
                  'Precision': float(scene_vals[scene_ind, 6]),
                  'Recall': float(scene_vals[scene_ind, 7]),
                  'F1': float(scene_vals[scene_ind, 8]),
                  'model_time': float(scene_vals[scene_ind, 9]),
                  'data_time': float(scene_vals[scene_ind, 10])},
            index=range(scene_ind, scene_ind + 1))

        data_frame.to_csv('F:/gwk/PointDSC-master/test_results2.csv',
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(
        columns=['scene', 'recall', 're', 'te', 'Precision', 'Recall', 'F1', 'model_time', 'data_time'])  # 列名
    df.to_csv('F:/gwk/PointDSC-master/test_results2.csv',
              mode='a', index=False, header=False)

    parser = argparse.ArgumentParser()
    parser.add_argument('--chosen_snapshot', default='PointDSC_3DMatch_06271224', type=str, help='snapshot dir')
    ...
    config_path = f'F:/gwk/PointDSC-master/snapshot/PointDSC_3DMatch_06271224/config.json'
    ...
    miss = model.load_state_dict(
        torch.load(f'F:/gwk/PointDSC-master/snapshot/PointDSC_3DMatch_06271224/models'
                   f'/model_best.pkl'), strict=False)
    print(miss)
    model.eval()

    # evaluate on the test set
    stats = eval_3DMatch(model.to(device), config, args.use_icp)
