...
class PointDSC(nn.Module):
    def __init__(self,
        ...
        self.matcher = SC2_PCR.Matcher(device=self.device,
    ...
    def forward(self, data):
        ...
        """
        corr_pos, src_keypts, tgt_keypts, gt_labels, epoch = data['corr_pos'], data['src_keypts'], data['tgt_keypts'], data['gt_labels'], data['epoch']
        # bs, num_corr = corr_pos.shape[0], corr_pos.shape[1]
        testing = 'testing' in data.keys()

        pred_trans, losses = self.matcher.SC2_PCR(corr_pos, src_keypts, tgt_keypts, testing, gt_labels, epoch)

        frag1_warp = transform(src_keypts, pred_trans)
